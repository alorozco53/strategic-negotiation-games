...
class RlAgent(RnnAgent):
    ...
    def read(self, inpt):
        ...
        self.words.append(self.model.word2var('THEM:').unsqueeze(0))
        self.words.append(Variable(inpt))
        self.words =  [(w.view(w.shape[0], 1)
                        if len(w.shape) < 2
                        else w) for w in self.words]
        assert (torch.cat(self.words).size(0) == torch.cat(self.lang_hs).size(0))
    ...
    def write(self, max_words):
        ...
        self.lang_hs.append(lang_hs)
        self.words.append(self.model.word2var('YOU:'))
        self.words.append(outs.view(outs.shape[0]))
        self.words = [(w.view(w.shape[0])
                       if len(w.shape) > 1
                       else w) for w in self.words]
        assert (torch.cat(self.words).size()[0] == torch.cat(self.lang_hs).size()[0])
        return self._decode(outs, self.model.word_dict)
    ...
    def update(self,
        ...
        partner_reward = partner_reward or 0

        diff = reward + partner_reward if self.args.cooperate else reward - partner_reward
        diff -= seller_reward or 0
        diff *= self.args.nu
        
        self.all_rewards.append(diff)
        r = (diff - np.mean(self.all_rewards)) / \
            max(1e-4, np.std(self.all_rewards))
    ...
    def expected_return(self,
        ...
        partner_reward = partner_reward \
                         if agree and partner_reward is not None \
                            else 0
        
        diff = reward + partner_reward if self.args.cooperate else reward - partner_reward
        diff -= self.args.nu * (seller_reward or 0)
        ...
        expected_return = 0
        for lp, r in zip(self.logprobs, rewards):
            expected_return -= lp * r
            
        return expected_return
        
        
    def update_lola(self,
                    agree,
                    reward,
                    partners,
                    choice=None,
        ...
        self.t += 1
        reward = reward #if agree else 0
        partner_reward = sum(partner_rewards) * self.args.nu \
                         if agree and partner_rewards is not None \
                            else 0
        ...
        self.all_rewards.append(diff)
        own_expected_return = self.expected_return(agree,
                                                   reward,
                                                   choice,
                                                   partner_reward,
                                                   seller_reward)
        partner_expected_returns = [p.expected_return(agree,
                                                      partner_rewards[i],
                                                      partner_choices[i],
                                                      reward,
                                                      seller_reward)
                                    for i, p in enumerate(partners)]
        partner_expected_return = sum(partner_expected_returns)
        double_expected_return = 0
        for lp in self.logprobs:
            double_expected_return -= partner_expected_return * lp
        
        
        loss = self.args.delta * (own_expected_return + double_expected_return * \
                                  own_expected_return * self.args.nu)
