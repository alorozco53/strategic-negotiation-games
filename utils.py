...
def load_model(file_name, cuda=True):
...
class ContextGenerator(object):
    ...
    def __init__(self, context_file, num_agents=2):
        self.ctxs = []
        with open(context_file, 'r') as f:
            ctx_tuple = []
            for line in f:
                ctx = line.strip().split()
                ctx_tuple.append(ctx)
                if len(ctx_tuple) == num_agents:
                    self.ctxs.append(ctx_tuple)
                    ctx_tuple = []
